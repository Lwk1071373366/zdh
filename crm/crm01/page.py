...
class PageNation:
    def __init__(self,base_url,current_page_num,request,total_counts,per_page_counts=10,page_number=5):
        '''
    ...
    def page_html(self):
        ...
        #首页
        self.params['page'] = 1
        shouye = '<li disabled><a href="{0}?{1}" aria-label="Previous" ><span aria-hidden="true">首页;</span></a></li>'.format(
        ...
        for i in range(self.page_start, self.page_end + 1):
            ...
            if self.current_page_num == i:
                ...
                one_tag = '<li class="active"><a href="{0}?page={1}">{1}</a></li>'.format(self.base_url, i,self.params.urlencode())
            else:
                one_tag = '<li><a href="{0}?page={1}">{1}</a></li>'.format(self.base_url, i,self.params.urlencode())
            tab_html += one_tag
        ...
        if self.current_page_num == self.page_number_count:
            ...
            self.params['page'] = self.current_page_num +1
            next_page = '<li><a href="{0}?page={1}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>'.format(self.base_url, self.params.urlencode())
        tab_html += next_page

        #尾页
        self.params['page'] = self.page_number_count
        weiye = '<li disabled><a href="{0}?{1}" aria-label="Previous" ><span aria-hidden="true">尾页;</span></a></li>'.format(
                self.base_url, self.params.urlencode())
        tab_html += weiye
        tab_html += '</ul></nav>'
